        ...
        <form action="{{ route('admin.movie.update',  $movie->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row mb-3">
                <div class="col-6">
                    ...
                    @enderror" value="{{$movie->age_rating}}">
                    @error('age_rating')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="col-6">
                    ...
                    <img src="{{ asset('storage/'.$movie->poster) }}" alt="" style="height: 150px; object-fit:cover;" class="d-block mx-auto mb-3">
                    <input type="file" name="poster" id="poster"
                        class="form-control @error('poster') is-invalid

                    @enderror" value="{{$movie->poster}}">
                    @error('poster')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
